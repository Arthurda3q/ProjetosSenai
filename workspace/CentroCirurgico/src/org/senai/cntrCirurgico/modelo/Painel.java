...
public class Painel {
	...
	public List<Painel> getLista(){
		...
		try {
			...
			while (rs.next()) {
				...
				p.setNomeCompleto(rs.getString("nome_completo"));//nome_da_coluna_no_banco
	...
	public Painel getPainel(int cod){
		...
		try {
			Connection conexao = new ConectarJDBC().getConectar();
			PreparedStatement ps = 
					conexao.prepareStatement("select * from painel where cod = ?");
			ps.setInt(1, cod);
			ResultSet rs = ps.executeQuery();
	...
	public String getCorStatus() {
		...
		if (status.contentEquals("Pré-Operatório")) {
			return "table-info";
		}
		if (status.contentEquals("Em Operação")) {
			return "table-warning";
		}
		if (status.contentEquals("Em Repouso")) {
			return "table-success";
		}
		if (status.contentEquals("Operacão Falha")) {
			return "table-danger";
		}
		return "";
